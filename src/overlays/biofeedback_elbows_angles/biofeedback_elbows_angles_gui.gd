...
# UI elements
@onready var node_aimings_list = %aimings_list
@onready var node_aiming_button = %aiming_button
...
func _ready():
	...
	create_item("coordinates.acromion_right")
	create_item("coordinates.lateral_epicondyl_right")
	create_item("coordinates.medial_epicondyl_right")
	create_item("coordinates.radial_styloid_right")
	create_item("coordinates.ulnar_styloid_right")
...
func _process(_delta: float) -> void:
	...
	if (
		Config.value_changed("biofeedback", "coordinates.acromion_right")
		or Config.value_changed("biofeedback", "coordinates.lateral_epicondyl_right")
		or Config.value_changed("biofeedback", "coordinates.medial_epicondyl_right")
		or Config.value_changed("biofeedback", "coordinates.radial_styloid_right")
		or Config.value_changed("biofeedback", "coordinates.ulnar_styloid_right")
	):
		update_values()
...
func _on_button_pressed():
	...
	if (
		coordinates == "coordinates.acromion_right"
		or coordinates == "coordinates.lateral_epicondyl_right"
		or coordinates == "coordinates.medial_epicondyl_right"
	):
		reference_frame_id = 302
	elif (
		coordinates == "coordinates.radial_styloid_right"
		or coordinates == "coordinates.ulnar_styloid_right"
	):
	...
	if node_optitrack.get_node(str(probe_id)) and node_optitrack.get_node(str(reference_frame_id)):
		...
		var inv_transform = node_frame_reference.global_transform.affine_inverse()

		# Transform probe position into the reference frame coordinate system
		pos = inv_transform.origin + inv_transform.basis * node_probe.position
