...
func _process(delta: float) -> void:
	...
	_target_speed = (0.0 if _should_stop else _slow_speed if _should_be_slow else _max_speed)
...
func _car_progress(delta: float) -> void:
	...
	for i in _triggers_on_curve.size():
		var offset_progress = (
			_path_follow.progress + _triggers_on_curve_offsets[i] + _current_speed / 2
		)
