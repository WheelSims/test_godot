...
func _process(_delta):
	if (
		...
		or Config.value_changed(
			"single_screen", "devices.screens.single_screen.full_screen.screen_index"
		)
	):
		if Config.get_value("devices.screens.single_screen.full_screen.enabled"):
			var screen_count = DisplayServer.get_screen_count()
			var screen_index = min(
				screen_count,
				Config.get_value("devices.screens.single_screen.full_screen.screen_index") - 1
			)
